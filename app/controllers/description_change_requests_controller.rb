class DescriptionChangeRequestsController < ValidationRequestsController
  ...
  def update
    if params["description_change_request"]["approved"] == "no" && params["description_change_request"]["rejection_reason"].blank?
      flash[:error] = "Please fill in the reason for disagreeing with the suggested change."
      render :edit
    elsif params["description_change_request"]["approved"] == "yes"
      send_approved_description(current_local_authority, params[:planning_application_id], params[:id], params[:change_access_id])
    elsif params["description_change_request"]["approved"] == "no"
      send_rejected_description(current_local_authority, params[:planning_application_id], params[:id], params[:change_access_id], params["description_change_request"]["rejection_reason"])
    end
  end
...
private
  ...
  def set_change_request
    @change_request = @change_requests["data"]["description_change_requests"].select { |obj| obj["id"] == params["id"].to_i }.first
  end
  ...
  def send_approved_description(subdomain, planning_application_id, description_change_request_id, change_access_id)
    request = HTTParty.patch(
      "#{ENV['PROTOCOL']}://#{api_base(subdomain)}/planning_applications/#{planning_application_id}/description_change_requests/#{description_change_request_id}?change_access_id=#{change_access_id}",
