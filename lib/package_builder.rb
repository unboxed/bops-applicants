...
private
  ...
  def application_name
    "#{ENV.fetch('AWS_DEPLOYMENT_APP_NAME', 'bops-applicants-app')}-#{environment}"
  end
  ...
  def remove_artifacts
    args = %w[rm -rf]
    args.concat %w[.bundle log storage tmp]
  ...
  def commit_url
    "https://github.com/unboxed/bops-applicants/commit/#{revision}"
  end
  ...
  def deployment_complete(deployment)
    ...
    if status == "succeeded"
      ...
      # Hard failure here so that if called as part of CI/CD the build fails
